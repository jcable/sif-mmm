...
{
var agree=confirm("Are you sure you wish to delete this listener?");
if (agree)
	return true ;
else
	return false ;
...
<td></table>
<h3>
SIF Project - Manage Listeners</h3>
<?php
	require 'connect.php';

	$result=mysql_query("SELECT * FROM listener order by listener asc", $connection);
	...
	while($row= mysql_fetch_array($result))
	{
		...
		print "\n<input type=\"hidden\" name=\"listener\" value=\"{$row["listener"]}\">";
		print "\n<td>{$row["listener"]}</td>";
